...
if __name__ == '__main__':
    ...
    for dataset_name in os.listdir('Datasets//'):    # dataset_name now contains the trailing '.csv'
        if(dataset_name.endswith('csv')):
            # Create files to write out the prints
            outfilename = "outputs//all_DT_10X5_5X5_dNoLim_10bags//" + dataset_name[:-4] +"_summary.txt"    
            outfile = open(outfilename,"w")
            
            csvfilename = "outputs//all_DT_10X5_5X5_dNoLim_10bags//" + dataset_name[:-4] +"_details.csv"     
    ...
    #            print("Grad %s" % accuracy_grad)
            ...
            outfile.write("\n\t\tBase\t\tRegular\t\tWagging\t\tMixed (opt)\tGrad-Mixed (opt)")
            outfile.write("\nAccuracy\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f" % 
            ...
            outfile.write("\nF1\t\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f" % 
            ...
            outfile.write("\nAUC\t\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f\t%.3f, %.3f" % 
            ...
            outfile.write("\n\nSiginificance tests:")
            outfile.write("\nFor Accuracy:")
            outfile.write("\n\tBase\tReg_bag\Wagging\tMix_bag\tGrad_mix")        # Divide p-value by 2 since we can use one-tailed test
            outfile.write("\nBase\t\t%.3f\t%.3f\t%.3f\t%.3f" % (acc_basereg_sigtest.pvalue, acc_basewag_sigtest.pvalue, acc_basemix_sigtest.pvalue, acc_basegrad_sigtest.pvalue))
            outfile.write("\nReg_bag\t\t\t%.3f\t%.3f\t%.3f" % (acc_regwag_sigtest.pvalue, acc_regmix_sigtest.pvalue, acc_reggrad_sigtest.pvalue))
            outfile.write("\nWagging\t\t\t\t\t%.3f\t%.3f" % (acc_wagmix_sigtest.pvalue, acc_waggrad_sigtest.pvalue))
            outfile.write("\nMix_bag\t\t\t\t\t\t%.3f" % (acc_mixgrad_sigtest.pvalue))
            
            outfile.write("\nFor F1:")
            outfile.write("\n\tBase\tReg_bag\tWagging\tMix_bag\tGrad_mix")        # Divide p-value by 2 since we can use one-tailed test
            outfile.write("\nBase\t\t%.3f\t%.3f\t%.3f\t%.3f" % (f1_basereg_sigtest.pvalue, f1_basewag_sigtest.pvalue, f1_basemix_sigtest.pvalue, f1_basegrad_sigtest.pvalue))
            outfile.write("\nReg_bag\t\t\t%.3f\t%.3f\t%.3f" % (f1_regwag_sigtest.pvalue, f1_regmix_sigtest.pvalue, f1_reggrad_sigtest.pvalue))
            ...
            outfile.write("\nBase\t\t%.3f\t%.3f\t%.3f\t%.3f" % (auc_basereg_sigtest.pvalue, auc_basewag_sigtest.pvalue, auc_basemix_sigtest.pvalue, auc_basegrad_sigtest.pvalue))
            outfile.write("\nReg_bag\t\t\t%.3f\t%.3f\t%.3f" % (auc_regwag_sigtest.pvalue, auc_regmix_sigtest.pvalue, auc_reggrad_sigtest.pvalue))
